...
begin
  if rising_edge(Clk) then
    ...
    if KbdInt = '1' then
      -- First, check if this is a release code prefix
      if KbdScanCode = x"F0" then 
        IsReleased <= '1'; 
      else 
        -- For all other scancodes, use the current IsReleased state
        -- Then clear it for next iteration
        ...
        IsReleased <= '0';  -- Clear release flag after processing key
      end if;
    end if;
